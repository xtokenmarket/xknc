...
describe('xKNC', () => {
	...
	describe('xKNC: deployment', () => {
		...
		it('should approve the staking contract to spend knc', async () => {
			...
			const approvedBal = await knc.allowance(xknc.address, kyberStaking.address);
	...
	describe('xKNC: minting with KNC', async () => {
		...
		it('should issue xKNC tokens to the caller', async () => {
			...
			await xknc.mintWithToken(utils.parseEther('0.01'));
	...
	describe('xKNC: DAO', async () => {
		...
		it('should not be able to claim if called from non-owner', async () => {
			await expect(xknc.connect(user).claimReward(1, [0], [10000], [0])).to.be.reverted;
		});

		it('should be able to add a fee handler for an ERC20', async () => {
			await xknc.addKyberFeeHandler(tokenKyberFeeHandler.address, mockToken.address);
	...
	describe('xKNC: Util Functions', async () => {
		...
		it('should not allow non admin to pause', async () => {
			await expect(xknc.connect(msigUser).pause()).to.be.reverted;
		});

		it('should allow admin to pause', async () => {
			await xknc.connect(manager2).pause();
			assert.isOk('Paused');
		});

		it('should not allow minting when paused', async () => {
			await expect(xknc.mint('0', { value: utils.parseEther('1') })).to.be.reverted;
		});

		it('should allow minting once unpaused', async () => {
			await xknc.connect(manager2).unpause();
			await xknc.mint('0', { value: utils.parseEther('1') });
			assert.isOk('Minted');
		});
	});
});
