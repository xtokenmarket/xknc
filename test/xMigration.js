...
describe('xMigration', () => {
  ...
  before(async () => {
    ...
    xkncaLegacy = await xKNCaLegacy.deploy(
    ...
    xmigration = await xMigration.deploy(
      xkncaLegacy.address,
      xknca.address,
      knc.address
    )
  ...
  it('should approve the migration contract to spend legacy xKNCa', async () => {
    ...
    const approvedBal = await xkncaLegacy.allowance(
      user1.address,
      xmigration.address
    )
  ...
  it('migrate legacy xKNCa', async () => {
    const xkncaLegacyBalBefore = await xkncaLegacy.balanceOf(user1.address)
    const xkncaBalBefore = await xknca.balanceOf(user1.address)
    const kncBalBefore = await knc.balanceOf(user1.address)

    await xmigration.connect(user1).migrate()

    const xkncaLegacyBalAfter = await xkncaLegacy.balanceOf(user1.address)
    assert.deepEqual(xkncaLegacyBalAfter, xkncaBalBefore, 'legacy xKNCa burnt: all 0')
    const xkncaBalAfter = await xknca.balanceOf(user1.address)
    assert.deepEqual(xkncaBalAfter, burnMintAmt, 'new xKNCa minted: all ' + burnMintAmt.toString())
  })

  it('should not able to migrate 0 balance', async () => {
    await expect(xmigration.connect(user1).migrate()).to.be.reverted
  })
